"""
MAATS Single-Agent workflow WITH TERMINOLOGY: Zero-shot (with terminology) -> Single agent evaluates all dimensions -> Single agent refinement.
...
Terminology is included conditionally in the zero-shot translation step (if available).
"""
...
def run_workflow(
    ...
    """
    Run MAATS single-agent translation workflow with terminology support.
    
    Terminology is included conditionally in the zero-shot translation step (if available).
    Some datasets don't have terminology dictionaries, so this is optional.
    ...
    Args:
        ...
        terminology: Optional terminology dictionary (used in zero-shot translation if available)
        region: AWS region
