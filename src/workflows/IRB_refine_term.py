"""
IRB-MT workflow WITH TERMINOLOGY: Two-stage self-refine translation.
Based on "IRB-MT at WMT25 Translation Task: A Simple Agentic System Using an Off-the-Shelf LLM"

Workflow:
1. Initial translation using base translation prompt WITH terminology dictionary
2. Refinement agent that reviews and improves the translation

The refinement prompt includes the original translation prompt (with terminology), source text, and initial translation.
...
def run_workflow(
    ...
    """
    Run IRB two-stage self-refine translation workflow with terminology support.
    
    Terminology is included in the initial translation prompt, and the refinement
    prompt includes the full original translation prompt (with terminology).
    ...
    Args:
        ...
        terminology: Optional terminology dictionary (used in initial translation)
        region: AWS region
    ...
    # Step 1: Initial translation (WITH terminology for IRB_refine_term)
    ...
    # Step 2: Refinement agent (includes the full translation prompt with terminology)
    print("    [Agent 2/2] Refinement...")
    refine_prompt = render_refine_prompt(
        translation_prompt=translation_prompt,  # Includes terminology if available
