"""
DeLTA (Document-levEL Translation Agent) workflow WITH TERMINOLOGY.
Based on "DELTA: AN ONLINE DOCUMENT-LEVEL TRANSLATION AGENT BASED ON MULTI-LEVEL MEMORY"

Implements Algorithm 1 from the paper with terminology dictionary support:
...
- Terminology Dictionary: Included in the document translator prompt
...
def render_document_translator_prompt_term(
    source_sentence: str,
    source_lang: str,
    target_lang: str,
    source_summary: str,
    ...
    """Render prompt for document translation with terminology."""
...
def run_workflow(
    ...
    """
    Run DeLTA document-level translation workflow with terminology support.
    
    Terminology is filtered for the entire document at the start and included
    in each translation step's prompt.
    ...
    Args:
        ...
        terminology: Optional terminology dictionary (used in translation prompts)
        region: AWS region
    ...
    # Filter terminology for the entire document at the start
    ...
    for i, source_sentence in enumerate(source_sentences):
        ...
        # Step 2: Translate with hybrid memory information and terminology (Algorithm 1, line 340)
        translate_prompt = render_document_translator_prompt_term(
            source_sentence=source_sentence,
