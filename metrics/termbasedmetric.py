...
try:
    from src.translation import create_openai_llm
    from src.vars import model_name2openai_id
except ImportError:
    from translation import create_openai_llm
    from vars import model_name2openai_id
...
class TermBasedMetric():
    ...
    Attributes:
        ...
        llm: ChatCDAO instance if aligner is 'llm' (uses CDAO instead of OpenAI).
    ...
    def _llm_prompt_alignment(self, src_sent, term, tgt_sent, fewshots):
        ...
        # Use CDAO instead of OpenAI
        # Combine system prompt (fewshots) with user query in a single HumanMessage
        # The original code uses system message (fewshots) + assistant message (query)
        # We combine them into a single HumanMessage for CDAO compatibility
        prompt = f"""{fewshots}

English sentence: {src_sent}
English term: {term}
{self.lang_tgt.capitalize()} translation: {tgt_sent}
Translated term: """
